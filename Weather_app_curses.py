...
def main():
    ...
    last_time = time.time() #datetime.datetime.now()

    while (True):
        now_time = time.time() # datetime.datetime.now()
    
        c = now_time - last_time
        ...
        #screen.addstr(3, 0, "Refreshes: " + str(refreshes))
        #screen.addstr(2, 40, "Millis: " + str(milliseconds)
        screen.addstr(4, 0, "FPS: " + str(1.0 / c))

        #-----------------------

        screen.addstr(1, 30, str(w.lat()) + " °")
        screen.addstr(2, 30, str(w.lon()) + " °")
        screen.addstr(3, 30, str(w.timezone()))
        screen.addstr(4, 30, str(w.timezone_offset()))

        screen.addstr(0, 50, "Current:")
        screen.addstr(1, 50, "Ido:\t\t" + str(datetime.datetime.fromtimestamp(w.current.dt())))
        screen.addstr(2, 50, "Napfelkelte:\t" + str(datetime.datetime.fromtimestamp(w.current.sunrise()).strftime('%H:%M:%S')))
        screen.addstr(3, 50, "Napnyugta:\t" + str(datetime.datetime.fromtimestamp(w.current.sunset()).strftime('%H:%M:%S')))


        screen.addstr(4, 50, "Hofok:\t" + str(w.current.temp()) + " °C")
        screen.addstr(5, 50, "Erzet:\t" + str(w.current.feels_like()) + " °C")
        screen.addstr(6, 50, "Legnyomas:\t" + str(w.current.pressure()) + " Hpa")
        screen.addstr(7, 50, "Paratartalom:\t" + str(w.current.humidity()) + " %")
        screen.addstr(8, 50, "Harmatpont:\t" + str(w.current.dew_point()) + " °C")
        screen.addstr(9, 50, "Eso:\t\t" + str(w.current.rain()) + " ")
        screen.addstr(10, 50, "Felho:\t" + str(w.current.clouds()) + " ")
        screen.addstr(11, 50, "Szel:\t\t" + str(w.current.wind_speed()) + " km/h")
        screen.addstr(12, 50, "Szelirany:\t" + str(w.current.wind_deg()) + " °")
    
        wind_speed_max = 0
        wind_speed_min = 0

        for wind_speed in range(12):
            if(w.hourly.wind_speed(wind_speed) > wind_speed_max):
                wind_speed_max = w.hourly.wind_speed(wind_speed)

        for o in range(12):
            screen.addstr(o, 100, str(w.hourly.wind_speed(o)))
            value = int(_map(w.hourly.wind_speed(o), wind_speed_min, wind_speed_max, 0, 20))    
            for x in range(value):
                screen.addstr(o, 105 + x, "#")
